...
int		initAirlineFromFile(Airline* pComp, AirportManager* pManaer, const char* fileName, const char* compressedFileName, int isCompress)
{
    ...
    if(isCompress == 1){
        if(loadAirlineFromCompressBFile(pComp,compressedFileName))
            return initDateList(pComp);
    }
...
int		saveAirlineToCompressBFile(const Airline* pComp, const char* fileName)
{
    ...
    BYTE data[2];
    data[0] = ((pComp->flightCount >>1)&0xFF);
    ...
    for (int i = 0; i < pComp->flightCount; i++)
    {
        if (!saveFlightToCompressBFile(pComp->flightArr[i], fp))
            return 0;
    }
                                 ...
                                 const char* fileName)
{
    ...
    pComp->flightCount = ((data[0] & 0xFF)<<1) | ((data[1]>>7) & 0x1);
    pComp->sortOpt = (eSortOption)(data[1] >> 4) &0x7;
    int len = data[1]&0xF;
    ...
    for (int i = 0; i < pComp->flightCount; i++)
    {
        ...
        if (!loadFlightFromCompressBFile(pComp->flightArr[i],  fp))
        {
            free(pComp->flightArr);
            fclose(fp);
            return 0;
