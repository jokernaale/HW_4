...
{
    int compress = 0;
    char* airlineFileName = AIRLINE_COMPRESS_FILE_NAME;
#ifdef COMPRESS
	...
	initManagerAndAirline(&manager, &company, AIRLINE_FILE_NAME,airlineFileName,compress);
    ...
    if(compress==1){
        saveAirlineToCompressBFile(&company,airlineFileName);
    }
...
int initManagerAndAirline(AirportManager* pManager, Airline* pCompany,const char* airlineFileName, const char* airlineCompressFileName, int ifCompress)
{
	...
	if (res == FROM_FILE)
		return initAirlineFromFile(pCompany, pManager, airlineFileName,airlineCompressFileName,ifCompress);
	else
